...
export const Checkout = () => {
  ...
  if (loading) {
    return (
      <div className={`h-[60vh] flex items-center justify-center transition-colors ${isDark ? 'bg-[#121212]' : 'bg-transparent'}`}>
        <Loader2 className={`w-8 h-8 animate-spin ${isDark ? 'text-gray-800' : 'text-gray-200'}`} />
      </div>
    );
  }

  if (cartProducts.length === 0 && !isSuccess) {
    return (
      <div className={`h-[60vh] flex flex-col items-center justify-center text-center transition-colors ${isDark ? 'bg-[#121212]' : 'bg-transparent'}`}>
        <h2 className={`text-2xl font-serif italic transition-colors ${isDark ? 'text-white' : 'text-gray-900'}`}>Your bag is empty</h2>
        <button 
          onClick={() => router.push('/')} 
          className={`mt-8 text-[10px] font-bold uppercase tracking-widest transition-colors ${isDark ? 'text-gray-500 hover:text-white' : 'text-gray-400 hover:text-black'}`}
        >
          Return to Archives
        </button>
      </div>
  ...
  return (
    <div className={`w-full min-h-screen transition-colors py-10 px-3 ${isDark ? 'bg-[#121212]' : 'bg-transparent'}`}>
                ...
                <div className="sticky top-32 space-y-12">
                    ...
                    <div className="space-y-6">
                        ...
                        <div key={item.id} className="flex gap-6">
                            ...
                            <div>
                                <h4 className={`text-[11px] font-bold uppercase tracking-tight truncate w-32 transition-colors ${isDark ? 'text-white' : ''}`}>{item.name}</h4>
                                <p className={`text-[10px] font-mono opacity-40 uppercase mt-1 ${isDark ? 'text-white' : ''}`}>QTY: {item.quantity}</p>
                            </div>
                            <span className={`text-[11px] font-bold font-mono transition-colors ${isDark ? 'text-white' : ''}`}>${(item.price * item.quantity).toFixed(2)}</span>
                            </div>
                        </div>
                        ))}
                    </div>
